...
function CreatePersistentWindow(key, options, routeLoader) {
  const store = new ElectronStore({ name: key });
  const saved = store.get("bounds");
  const windowOptions = {
    ...options,
    width: (saved == null ? void 0 : saved.width) || options.width,
    height: (saved == null ? void 0 : saved.height) || options.height,
    x: saved == null ? void 0 : saved.x,
    y: saved == null ? void 0 : saved.y
  };
...
let loadingWindow;
let tray;
const userDataWinSize = { width: 1e3, height: 800, minWidth: 491, minHeight: 300 };
const userSettingsWinSize = { horizontalWidth: 1e3, horizontalHeight: 439, verticalWidth: 700, verticalHeight: 697 };
...
function createDataWindow() {
  ...
  dataWindow.webContents.on("did-finish-load", () => {
    dataWindow == null ? void 0 : dataWindow.webContents.send("main-process-message", (/* @__PURE__ */ new Date()).toLocaleString());
  });
...
function createInputWindow() {
  ...
  }, userInputSettings.questionScreenTime * 1e3);
...
function createTrayIcon() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { label: "Logout", type: "normal", click: () => ipcMain$1.emit("logout") },
    { label: "Exit", type: "normal", click: () => app$1.quit() }
  ]);
...
function checkShowInput() {
  ...
  if (currentTime - lastShown >= userInputSettings.questionInterval * 6e4) {
...
function setupLinuxAutostart() {
  const autostartDir = path.join(os$1.homedir(), ".config", "autostart");
  if (!fs$1.existsSync(autostartDir)) fs$1.mkdirSync(autostartDir, { recursive: true });
  const desktopFile = `
    [Desktop Entry]
    Type=Application
  ...
  `;
  fs$1.writeFileSync(path.join(autostartDir, "activity-logger.desktop"), desktopFile);
}
app$1.on("activate", () => {
...
ipcMain$1.on("close-window", (event) => {
  const window = BrowserWindow.fromWebContents(event.sender);
  window == null ? void 0 : window.close();
});
ipcMain$1.on("minimize-window", (event) => {
  const window = BrowserWindow.fromWebContents(event.sender);
  window == null ? void 0 : window.minimize();
});
ipcMain$1.on("maximize-window", (event) => {
  const window = BrowserWindow.fromWebContents(event.sender);
  if (window == null ? void 0 : window.isMaximized()) {
    window.unmaximize();
  } else {
    window == null ? void 0 : window.maximize();
  }
});
ipcMain$1.on("login-success", () => {
  if (loadingWindow) {
    loadingWindow.close();
    loadingWindow = null;
  }
...
ipcMain$1.on("change-settings-layout", (_, mode) => {
...
ipcMain$1.on("update-user-input-settings", (_, settings) => {
  userInputSettings = settings;
  lastShown = Date.now();
  setupTimer();
});
app$1.on("window-all-closed", (event) => {
